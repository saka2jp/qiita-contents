...
12 日目、お疲れ様です！
Phase 2 も佳境に入ってきました。今日は CSS における「錬金術」とも呼べる、疑似要素（Pseudo-elements）です。

これまで、何か模様をつけたいときは HTML に`div`タグを追加していたかもしれません。
しかし、疑似要素を使えば、 **HTML には何も書かずに、CSS だけで「幽霊」のような要素を生み出して配置する** ことができます。

コードを汚さずにリッチな装飾を作る、プロ御用達の必須テクニックです！
...
リンクをホバーした時に、左から右へラインが伸びるおしゃれな動き。これも疑似要素で作ります。
...
疑似要素を使えば、この役割分担を完璧に守りつつ、デザイン性を高めることができます。
「ここにちょっと飾りが欲しいな」と思ったら、HTML をいじる前に「`::before` でいけるかも？」と考えてみてください。

明日の Day 13 は、写真を CSS だけで加工する **「画像加工 (object-fit / filter)」** です。Photoshop いらずの技術を身につけましょう！
