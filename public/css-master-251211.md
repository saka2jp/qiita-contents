...
11 日目、お疲れ様です！
Phase 2 も中盤、昨日は「形」を自由自在に操れるようになりました。

今日は Web デザインの**9 割**を占めると言われる「文字（タイポグラフィ）」です。
「ただ文字を表示する」のと「読ませる・魅せる」のは全く別物です。
フォント選びとちょっとした CSS の調整だけで、素人っぽいサイトが一瞬でプロ級のデザインに変わります。

今日は、文字を **「読む情報」から「魅せるデザイン」に昇華** させましょう！
...
2.  **`line-height`（行間）** と **`letter-spacing`（字間）** で、「読みやすさ」を劇的に向上させる。
...
- **`line-height` (行の高さ):**
  - デフォルトは狭すぎます。
  - 本文なら **`1.6` 〜 `1.8`** くらいに設定すると、一気にプロっぽく読みやすくなります。単位（px や%）をつけずに数字だけで指定するのがベストプラクティスです。
...
`text-shadow` を使うと影を落とせますが、モダンな Web デザインでは「中抜き文字（ストローク）」も人気です。
...
背景画像の上にこれを配置すると、背景が文字の中を透けて見えて非常にかっこいいです。
...
たかが文字、されど文字。
「フォントを変える」「行間を空ける」。この 2 つを意識するだけで、あなたの作るサイトのクオリティは劇的に上がります。
特にポートフォリオなどでは、文字の美しさがそのまま「几帳面さ」として評価されますよ。

さて、明日の Day 12 は **「疑似要素 (::before / ::after)」** です。
HTML には何も書かずに、CSS だけでアイコンや装飾を出現させる「錬金術」を学びます。これぞ CSS の真骨頂です！
