...
9 日目、おはようございます！
昨日の「グラデーション」で平面に色が着きましたね。
今日はそこに「**奥行き**」を与えます。

画面は 2 次元ですが、人間は「影」があるだけでそれを「物体」として認識します。
CSS の `box-shadow` は、ただ影を落とすだけでなく、**物体を浮かせたり、へこませたり、光らせたり**できる魔法のプロパティです。

今日はデザインの流行を作ってきた「影の魔術」を習得しましょう！
...
この「高さ」をコントロールできるようになると、ユーザーに「ここを押して！」と無言で伝えられるようになります。
...
2.  **マテリアルデザイン風**：紙がふわりと浮いているような自然な影を作る。
...
3.  **Blur (20px):** ぼかし具合。0 だとくっきり、数値が大きいとボワッとする。
4.  **Spread (5px):** 影の拡大・縮小。省略可能。
5.  **Color:** 影の色。`rgba`を使って透明度を指定するのが鉄則です。真っ黒（#000）を使うと汚く見えるので注意！
...
「影」をコントロールできると、UI デザインのクオリティが一気にプロっぽくなります。
特に **`rgba(0,0,0,0.1)` のような「薄い影」を「大きくぼかす」** のが、最近のおしゃれな Web サイトのトレンドです。

明日の Day 10 は、四角い箱から卒業しましょう。
「**ボーダーと角丸**」を使って、有機的な「歪んだ形」や美しい円を作るテクニックです！お楽しみに！
