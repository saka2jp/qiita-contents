...
今日は、Web ＝四角形という常識をぶち壊します。
CSS の「角丸」を極めれば、真円はもちろん、カプセル型、さらにはアメーバのような**有機的でぐにゃぐにゃした形（Blob）**まで作れるようになります。

---

### はじめに：ただの「角丸」だと思って侮るな

`border-radius` は、単にボタンの角を少し丸くするためだけのプロパティではありません。
4 つの角の値をバラバラに設定したり、縦と横の比率を変えたりすることで、**CSS だけで複雑な図形を描画する**ことができます。

画像を使わずに形を作れれば、色やサイズの変更も CSS だけで一瞬で終わります。
...
ボタンの高さをいちいち測って `height: 50px; border-radius: 25px;` と書くのは面倒ですよね？
`border-radius: 9999px`（または十分大きな値）を指定すると、ブラウザが勝手に**「短い方の辺の半分」**を計算して、綺麗な半円を作ってくれます。

#### 3. Blob（歪んだ形）の正体

サンプルコードの Mission 3 にある `60% 40% ... / ...` という書き方。
これは「**水平方向の半径 / 垂直方向の半径**」を別々に指定しています。これにより、真円ではない、歪んだ曲線を作ることができます。
これと CSS アニメーションを組み合わせると、まるで生き物のように動く背景が作れます。
...
特に「Blob」は、写真の背景に薄く敷いたり、ボタンの背景に使ったりすると、一気に今風のデザインになります。

さて、形ができたら次は中身です。
明日の Day 11 は、Web デザインの要である「**テキスト装飾と Web フォント**」です。
文字をただ並べるだけでなく、美しく魅せる技術を学びましょう！
